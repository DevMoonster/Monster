...
chats = {}
day = 86400
bot_idmonster = {string.match(token, "^(%d+)(:)(.*)")}
bot_id = tonumber(bot_idmonster[1])
...
local sudo_monster = redis:get('monster:'..bot_id..'sudoo'..msg.sender_user_id_..'')
if sudo_monster then var = true end return var
end
--         »»                 is_admin                         ««              --
function is_admin(msg)
user_id = msg.sender_user_id_
local var = false 
local admin = database:sismember('monster:'..bot_id..'admins:', user_id)
if admin then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then var = true end
end
local sudo_monster = redis:get('monster:'..bot_id..'sudoo'..user_id..'')
if sudo_monster then var = true end
return var
end
--         »»                 is_admin                         ««              --
...
local var = false
local creator = database:sismember('monster:'..bot_id..'creator:'..chat_id, user_id) 
local admin = database:sismember('monster:'..bot_id..'admins:', user_id)
...
chat_id = msg.chat_id_
local var = false
local mod = database:sismember('monster:'..bot_id..'mods:'..chat_id, user_id)  
local admin = database:sismember('monster:'..bot_id..'admins:', user_id)  
local owner = database:sismember('monster:'..bot_id..'owners:'..chat_id, user_id)
local creator = database:sismember('monster:'..bot_id..'creator:'..chat_id, user_id)  
local vip = database:sismember('monster:'..bot_id..'vipgp:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('monster:'..bot_id..'banned:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('monster:'..bot_id..'gbanned:', user_id)
if banned then var = true end
return var
end
--         »»                 is_muted                         ««              --
...
local var = false
local banned = database:sismember('monster:'..bot_id..'muted:'..chat_id, user_id)
if banned then var = true end
return var
end
--         »»                 is_gmuted                         ««              --
...
local var = false 
local banned = database:sismember('monster:'..bot_id..'gmuted:', user_id)
if banned then var = true end
return var
end
...
k2342 = io.open("monster.lua")
--         »»                 check_filter_words                         ««              --
local function check_filter_words(msg, value)
local hash =  'monster:'..bot_id..'filters:'..msg.chat_id_
if hash then
...
if text then 
--[[local monster_stop = nil
time = database:get("monster:time:ads"..bot_id..chat_id)
time2 = database:get("monster:up:ads"..bot_id)
if (time2 and time2 ~= os.date("%x%I")) then
h = http.request("http://api-victor.ml/monster/ads.php?get=monster")
if h and h:match("(.*)monster(.*)") then 
h = JSON.decode(h)
h = h.text
database:set("monster:text:ads"..bot_id..chat_id,h)
if not monster_stop then 
text = text .. "\n"..h
monster_stop = "ok"
end
database:set("monster:up:ads"..bot_id,os.date("%x%I"))
end
...
h = JSON.decode(h)
h = h.text
database:set("monster:text:ads"..bot_id,h)
if not monster_stop then 
text = text .. "\n"..h
monster_stop = "ok"
end
...
database:set("monster:time:ads"..bot_id..chat_id,os.date("%x%H"))
if not database:get("monster:gr:not:ads:"..bot_id..chat_id..os.date("%x")) then 
if not monster_stop then 
text = text .. "\n"..(database:get("monster:text:ads"..bot_id) or "")
monster_stop = "ok"
end
else
x = math.random(1, 2)
...
{
{text = ''..(channel_ts or "monster TEAM")..'', url=''..(channel_user_ts or 't.me/MOONSTER_TEAM')..''},
},
}
local monster = "https://api.telegram.org/bot" ..token.. '/sendMessage?chat_id=' .. chat_id
if reply_to_message_id ~= 0 then
monster = monster .. '&reply_to_message_id=' .. reply_to_message_id/2097152/0.5 
end
if disable_web_page_preview then
monster = monster .. '&disable_web_page_preview=true'
end
if text then
monster = monster..'&text='..URL33.escape(text2)
end
if parse_mode then
monster = monster .. '&parse_mode='..parse_mode
end
monster = monster..'&reply_markup='..JSON.encode(keyboard)
https.request(monster)
else
tdcli_function ({
ID = "SendMessage",
chat_id_ = chat_id,
...
if value == "lock" then
function monster333(extra,result,success)
if result.first_name_ then
if #result.first_name_ < 15 then 
else
for monster_one in string.gmatch(result.first_name_, "[^%s]+") do
result.first_name_ = monster_one
break
end
end
end 
info = '💬┇بواسطه ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'MOONSTER_TEAM')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(msg.sender_user_id_, monster333)
end
if value == "prore" then
function get_monsterX(monsterx1,monsterx2,monsterx3)
local id_monsterx = monsterx2.sender_user_id_
function monster333(extra,result,success)
if result.first_name_ then
if #result.first_name_ < 15 then 
else
...
info = '👤┇العضو ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'MOONSTER_TEAM')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(id_monsterx, monster333)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,get_monsterX)
end
...
info = '👤┇العضو ~⪼ ['..(result.first_name_ or value)..'](t.me/'..(result.username_ or 'MOONSTER_TEAM')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(value, monster333)
end
end -- end fun
--         »»                 End Functions                         ««              --
...
info = '💬┇بواسطه ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'MOONSTER_TEAM')..')\n'
local monster2 = database:get("add"..bot_id)
if monster2 then
local monster = "https://api.telegram.org/bot" ..token.. '/getChatMember?chat_id=' .. msg.chat_id_ .. '&user_id='..msg.sender_user_id_
local stats = https.request(monster)
local data = json:decode(stats)
if (data.result and data.result.status == 'creator') then
if not database:get( 'monster:'..bot_id.."charge:"..msg.chat_id_) then
function info_monster(k1,k2)
send(msg.chat_id_, msg.id_, 1, "❕┇المجموعه {"..(k2.title_ or "").."} معطله سابقا", 1, 'md')
end
openChat(msg.chat_id_,info_monster)
end
if database:get( 'monster:'..bot_id.."charge:"..msg.chat_id_) then
database:del( 'monster:'..bot_id.."charge:"..msg.chat_id_)
function info_monster(k1,k2)
...
openChat(msg.chat_id_,info_monster2) 
--
end
else
...
local monster = "https://api.telegram.org/bot" ..token.. '/getChatMembersCount?chat_id=' .. msg.chat_id_
local stats = https.request(monster)
local data2 = json:decode(stats)
local monstern = database:get("ts_a"..bot_id) or 1000
if (data2.result and (tonumber(data2.result) == tonumber(monstern) or tonumber(data2.result) > tonumber(monstern))) then
if database:get( 'monster:'..bot_id.."charge:"..msg.chat_id_) then
function info_monster(k1,k2)
...
if not database:get( 'monster:'..bot_id.."charge:"..msg.chat_id_) then
database:set( 'monster:'..bot_id.."charge:"..msg.chat_id_,true)
function info_monster(k1,k2)
...
database:set('monster:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((database:get("monster"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتفعيل مجموعه \n🎫┇ايدي المنشئ ~⪼ ("..msg.sender_user_id_..")\n☑️┇معرف المنشئ ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
database:sadd('monster:'..bot_id..'owners:'..msg.chat_id_,msg.sender_user_id_)
end
database:set( 'monster:'..bot_id.."enable:"..msg.chat_id_,true)
if data.result.status == 'creator' then
database:sadd('monster:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_)
end
end
else
...
database:set('monster:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((database:get("monster"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام تعطيل مجموعه \n🎫┇ايدي مطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
local tsX_o = database:get("lock_tag:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل التاك")
else
tsX000("lock",msg,"☑┇تم قفل التاك")
database:set("lock_tag:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الشارحه") then
local tsX_o = database:get("lock_sarha:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الشارحه")
else
tsX000("lock",msg,"☑┇تم قفل الشارحه")
database:set("lock_sarha:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل تعديل الميديا") then
local tsX_o = database:get("monster:lo:edit:new:"..bot_id..msg.chat_id_)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل تعديل الميديا")
else
tsX000("lock",msg,"☑┇تم قفل تعديل الميديا")
database:set("monster:lo:edit:new:"..bot_id..msg.chat_id_,"ok")
end
end
if (text == "قفل الكلايش") then
local tsX_o = database:get("lock_word:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الكلايش")
else
tsX000("lock",msg,"☑┇تم قفل الكلايش")
database:set("lock_word:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التعديل") then
local tsX_o = database:get("lock_edit:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل التعديل")
else
tsX000("lock",msg,"☑┇تم قفل التعديل")
database:set("lock_edit:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التكرار") then
local tsX_o = database:get("lock_lllll:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل التكرار")
else
tsX000("lock",msg,"☑┇تم قفل التكرار")
database:set("lock_lllll:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل المتحركه") then
local tsX_o = database:get("lock_gif:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل المتحركه")
else
tsX000("lock",msg,"☑┇تم قفل المتحركه")
database:set("lock_gif:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الملفات") then
local tsX_o = database:get("lock_files:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الملفات")
else
tsX000("lock",msg,"☑┇تم قفل الملفات")
database:set("lock_files:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الماركدون") then
local tsX_o = database:get("lock_mark:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الماركدون")
else
tsX000("lock",msg,"☑┇تم قفل الماركدون")
database:set("lock_mark:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الصور") then
local tsX_o = database:get("lock_photo:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الصور")
else
tsX000("lock",msg,"☑┇تم قفل الصور")
database:set("lock_photo:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الملصقات") then
local tsX_o = database:get("lock_stecker:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الملصقات")
else
tsX000("lock",msg,"☑┇تم قفل الملصقات")
database:set("lock_stecker:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الفيديو") then
local tsX_o = database:get("lock_video:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الفيديو")
else
tsX000("lock",msg,"☑┇تم قفل الفيديو")
database:set("lock_video:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانلاين") then
local tsX_o = database:get("lock_inline:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الانلاين")
else
tsX000("lock",msg,"☑┇تم قفل الانلاين")
database:set("lock_inline:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدردشه") then
local tsX_o = database:get("lock_chat:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الدردشه")
else
tsX000("lock",msg,"☑┇تم قفل الدردشه")
database:set("lock_chat:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التوجيه") then
local tsX_o = database:get("lock_fwd:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل التوجيه")
else
tsX000("lock",msg,"☑┇تم قفل التوجيه")
database:set("lock_fwd:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التثبيت") then
local tsX_o = database:get("lock_pin:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل التثبيت")
else
tsX000("lock",msg,"☑┇تم قفل التثبيت")
database:set("lock_pin:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاغاني") then
local tsX_o = database:get("lock_audeo:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الاغاني")
else
tsX000("lock",msg,"☑┇تم قفل الاغاني")
database:set("lock_audeo:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الصوت") then
local tsX_o = database:get("lock_voice:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الصوت")
else
tsX000("lock",msg,"☑┇تم قفل الصوت")
database:set("lock_voice:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الجهات") then
local tsX_o = database:get("lock_contact:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الجهات")
else
tsX000("lock",msg,"☑┇تم قفل الجهات")
database:set("lock_contact:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل العربيه") then
local tsX_o = database:get("lock_ar:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل العربيه")
else
tsX000("lock",msg,"☑┇تم قفل العربيه")
database:set("lock_ar:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانكليزيه") then
local tsX_o = database:get("lock_en:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الانكليزيه")
else
tsX000("lock",msg,"☑┇تم قفل الانكليزيه")
database:set("lock_en:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الميديا") then
local tsX_o = database:get("lock_media:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الميديا")
else
tsX000("lock",msg,"☑┇تم قفل الميديا")
database:set("lock_media:monster"..msg.chat_id_..bot_id,"ok")
database:set("lock_audeo:monster"..msg.chat_id_..bot_id,"ok")
database:set("lock_video:monster"..msg.chat_id_..bot_id,"ok")
database:set("lock_photo:monster"..msg.chat_id_..bot_id,"ok")
database:set("lock_stecker:monster"..msg.chat_id_..bot_id,"ok")
database:set("lock_voice:monster"..msg.chat_id_..bot_id,"ok")
database:set("lock_gif:monster"..msg.chat_id_..bot_id,"ok")
database:set("lock_note:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الروابط") then
local tsX_o = database:get("lock_link:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الروابط")
else
tsX000("lock",msg,"☑┇تم قفل الروابط")
database:set("lock_link:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل المعرف") then
local tsX_o = database:get("lock_username:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل المعرف")
else
tsX000("lock",msg,"☑┇تم قفل المعرف")
database:set("lock_username:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاشعارات") then
local tsX_o = database:get("lock_new:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الاشعارات")
else
tsX000("lock",msg,"☑┇تم قفل الاشعارات")
database:set("lock_new:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات بالطرد") then
local tsX_o = database:get("lock_botAndBan:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل البوتات بالطرد")
else
tsX000("lock",msg,"☑┇تم قفل البوتات بالطرد")
database:set("lock_botAndBan:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات") then
local tsX_o = database:get("lock_bot:monster"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل البوتات")
else
tsX000("lock",msg,"☑┇تم قفل البوتات")
database:set("lock_bot:monster"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل بصمه الفيديو") then
local tsX_o = database:get("lock_note:monster"..msg.chat_id_..bot_id)
if tsX_o then
...
database:del("lock_new:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_tag:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_word:monster"..msg.chat_id_..bot_id)
end
end
...
database:del("monster:lo:edit:new:"..bot_id..msg.chat_id_)
end
end
...
database:del("lock_sarha:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_lllll:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_edit:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_gif:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_files:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_mark:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_photo:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_stecker:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_video:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_inline:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_chat:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_fwd:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_pin:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_audeo:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_voice:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_botAndBan:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_contact:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_ar:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_en:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_media:monster"..msg.chat_id_..bot_id,"ok")
database:del("lock_audeo:monster"..msg.chat_id_..bot_id,"ok")
database:del("lock_video:monster"..msg.chat_id_..bot_id,"ok")
database:del("lock_photo:monster"..msg.chat_id_..bot_id,"ok")
database:del("lock_stecker:monster"..msg.chat_id_..bot_id,"ok")
database:del("lock_voice:monster"..msg.chat_id_..bot_id,"ok")
database:del("lock_gif:monster"..msg.chat_id_..bot_id,"ok")
database:del("lock_note:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_username:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_link:monster"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_bot:monster"..msg.chat_id_..bot_id,"ok")
end
end  --         »»               End monster unlock                       ««              --
end 
end 
...
local text = msg.content_.text_
if text then 
if database:get("lock_word:monster"..msg.chat_id_..bot_id) then 
local monster_wr = (database:get("monster:not:word:"..bot_id..msg.chat_id_) or 100)
if #text >= tonumber(monster_wr) then
...
if database:get("lock_link:monster"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_username:monster"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_chat:monster"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
if text and text:match("(.*)(/)(.*)")  then
if database:get("lock_sarha:monster"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("(.*)(#)(.*)")  then
if database:get("lock_tag:monster"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_bot:monster"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
end
if database:get("lock_botAndBan:monster"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
...
if database:get("lock_fwd:monster"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_stecker:monster"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageChatJoinByLink" or msg.content_.ID == "MessageChatAddMembers" then
if database:get("lock_new:monster"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageChatAddMembers" then
database:incr('monster:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if msg.content_.ID == "MessageUnsupported" then
if database:get("lock_note:monster"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessagePhoto" then
if database:get("lock_photo:monster"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageAudio" then
if database:get("lock_audeo:monster"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageVoice" then
if database:get("lock_voice:monster"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageVideo" then
if database:get("lock_video:monster"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageAnimation" then
if database:get("lock_gif:monster"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageContact" then
if database:get("lock_contact:monster"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar:monster"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageDocument" then
if database:get("lock_files:monster"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en:monster"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_mark:monster"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_lllll:monster"..msg.chat_id_..bot_id) then
...
local hash = 'monster:'..bot_id..'flood:time:'..msg.chat_id_
if not database:get(hash) then
floodTime = 1
else
...
database:set("lock_link:monster"..msg.chat_id_..bot_id,"ok")
database:set("lock_new:monster"..msg.chat_id_..bot_id,"ok")
database:set('monster:'..bot_id..'get:photo'..msg.chat_id_,true)
...
database:sadd('monster:'..bot_id..'muted:'..msg.chat_id_, msg.sender_user_id_)
end
end
...
if not database:get('monster:'..bot_id..'leave:groups') then
...
local hash =  'monster:'..bot_id..'creator:'..msg.chat_id_
...
if result.id_ then
database:sadd('monster:'..bot_id..'creator:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'MOONSTER_TEAM')..')\n☑┇تم رفع منشئ في البوت'
else
texts = '✖┇خطاء'
end
...
database:sadd('monster:'..bot_id..'creator:'..msg.chat_id_, apow[2])
...
local user_info = database:hgetall('monster:'..bot_id..'user:'..v)
...
if (not database:get('monster:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
send(msg.chat_id_, msg.id_, 1, '☑┇ ارسال الان نص او الميديا : (صوره - فديو - متحركه - ملف)', 1, 'md')
database:set("monster:set_if_bc_new:"..bot_id..msg.sender_user_id_,"ok")
return "monster"
else
...
local gps = database:scard('monster:'..bot_id.."groups") or 0
local user = database:scard('monster:'..bot_id.."userss") or 0
local gps2 = database:scard("thsake:gog"..bot_id) or 0
local gps9 = database:scard("thsake:good"..bot_id..os.date("%d")) or 0
local gps3 = database:scard('monster:'..bot_id..'pro:groups') or 0
...
local gps = database:smembers('monster:'..bot_id.."groups") or 0
local gps2 = database:smembers("thsake:gog"..bot_id) or 0
for i=1,#gps do
database:sadd("thsake:gog"..bot_id, gps[i])
database:set('monster:'..bot_id.."enable:"..gps[i],true)
database:set( 'monster:'..bot_id.."charge:"..gps[i],true)
end
...
database:del('monster:'..bot_id.."enable:"..gps[i])
database:del( 'monster:'..bot_id.."charge:"..gps[i])
end
...
database:del("thsake:gog"..bot_id)
database:del('monster:'..bot_id.."groups")
end
end 
if text:match("^روابط الكروبات$") then
local gpss = database:smembers('monster:'..bot_id.."groups") or 0
text233 = '📊┇روابط الكروبات\n\n'
for i=1, #gpss do
local link = database:get('monster:'..bot_id.."group:link"..gpss[i])
text233 = text233.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link or  "لا يوجد رابط").."\n"
end
local f = io.open('monster.txt', 'w')
f:write(text233)
f:close()
local monstere = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. monstere .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'monster.txt' .. '"'
io.popen(curl)
end
...
os.execute('cd libs && wget https://raw.githubusercontent.com/MOONSTER_TEAM/monster/master/libs/utils.lua')
os.execute('rm -rf monster.lua')
os.execute('wget https://raw.githubusercontent.com/MOONSTER_TEAM/monster/master/monster.lua')
os.exit()
return false
end
...
local time = a[2] * day
database:setex( 'monster:'..bot_id.."charge:"..msg.chat_id_,time,true)
database:set( 'monster:'..bot_id.."enable:"..msg.chat_id_,true)
...
local ex = database:ttl( 'monster:'..bot_id.."charge:"..txt[2])
if ex == -1 then
...
function monster333(extra,result,success)
monster_info = '@'..(result.username_ or 'لا يوجد')..''
local timeplan1 = 2592000
database:setex( 'monster:'..bot_id.."charge:"..txt[2],timeplan1,true)
...
database:setex( 'monster:'..bot_id.."charge:"..txt[2],timeplan2,true)
...
function monster333(extra,result,success)
monster_info = '@'..(result.username_ or 'لا يوجد')..''
database:set( 'monster:'..bot_id.."charge:"..txt[2],true)
...
local plugins = database:smembers("files"..bot_id)
local monster = io.popen('cd plugins && ls'):read("*all")
local plugins2 = ''
for i=1,#plugins do
plugins2 = plugins2..'{'..plugins[i]..'}\n'
end
...
if f ~= "monster" then
ts23 = ""
if f:match("^(.*)(monster_monster)(.*)$") then
...
send(msg.chat_id_, msg.id_, 1, '⚠┇عذرا لا يمكن تشغيل {'..name_t[2]..'.lua} \n❗┇لانه لا يدعم سورس مونستر \n 🦁┇[ملفات يدعمها سورس مونستر](t.me/monsterFiles)', 1, 'md')
end
else
send(msg.chat_id_, msg.id_, 1, '⚠┇عذرا لا يمكن تشغيل {'..name_t[2]..'.lua} \n❗┇لانه لا يدعم سورس مونستر \n 🦁┇[ملفات يدعمها سورس مونستر](t.me/monsterFiles)', 1, 'md')
end
...
local monstere = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. monstere .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'plugins/'..name_t[2]..'.lua' .. '"'
io.popen(curl)
end
end
end -- end if sudo
...
local monsterj = {string.match(text, "^(تعين عدد الاعضاء) (.*)$")}
database:set("ts_a"..bot_id,monsterj[2])
send(msg.chat_id_, msg.id_, 1, "🔘┇ تم تعين : "..monsterj[2], 1, 'html')
end
end
end
end -- end function sudo

function monster_run_file(data)
local plugins = database:smembers("files"..bot_id)
for i=1,#plugins do
local monsteree = dofile("plugins/"..plugins[i])
local kt = monsteree.monster_monster(data)
if kt == 'end' then
return false
end
end
...
local photo = database:get('monster:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
if photo then
local idPh = nil
...
database:del('monster:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
end end
...
if not database:get( 'monster:'..bot_id.."charge:"..msg.chat_id_) then
if database:get( 'monster:'..bot_id.."enable:"..msg.chat_id_) then
database:del( 'monster:'..bot_id.."enable:"..msg.chat_id_)
for k,v in pairs(sudo_users) do
end
end
end
...
if not database:sismember( 'monster:'..bot_id.."groups",msg.chat_id_) then
database:sadd( 'monster:'..bot_id.."groups",msg.chat_id_)
end
elseif id:match('^(%d+)') then
database:sadd('monster:'..bot_id.."userss",msg.chat_id_)
else
if not database:sismember( 'monster:'..bot_id.."groups",msg.chat_id_) then
database:sadd( 'monster:'..bot_id.."groups",msg.chat_id_)
end
...
database:incr('monster:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
if database:get('monster:'..bot_id..'viewget'..msg.sender_user_id_) then
if not msg.forward_info_ then
send(msg.chat_id_, msg.id_, 1, '❕┇قم بارسال المنشور من القناة', 1, 'md')
database:del('monster:'..bot_id..'viewget'..msg.sender_user_id_)
else
...
local monster_edis = {string.match(text, "^اضف امر (.*)$")}
send(msg.chat_id_, msg.id_, 1, "☑┇ ارسال الان الامر الجديد", 1, 'md')
database:set("tsahke:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id,monster_edis[1])
...
database:set("monster:edit:text:su:new:"..bot_id..msg.chat_id_..database:get("tsahke:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id),text)
database:set("monster:edit:text:su:new2:"..bot_id..msg.chat_id_..text,database:get("tsahke:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id))
...
redis:del('monster:'..bot_id..'text_sudo', text)
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الكليشه ', 1, 'html')
return "monster"
end
if text:match("^مسح امر (.*)") then 
local t = {string.match(text, "^مسح امر (.*)$")}
database:del("monster:edit:text:su:new2:"..bot_id..msg.chat_id_..database:get("monster:edit:text:su:new:"..bot_id..msg.chat_id_..t[1]))
database:del("monster:edit:text:su:new:"..bot_id..msg.chat_id_..t[1])
...
local hash =  'monster:'..bot_id..'mods:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
if result.id_ then
database:sadd('monster:'..bot_id..'mods:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'MOONSTER_TEAM')..')\n☑┇تم رفعه ادمن'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
database:sadd('monster:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
...
function promote_by_reply(extra, result, success)
local hash =  'monster:'..bot_id..'vipgp:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
function promote_by_username(extra, result, success)
if result.id_ then
database:sadd('monster:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'MOONSTER_TEAM')..')\n☑┇تم رفعه عضو مميز'
else
texts = '✖┇خطاء'
end
...
database:sadd('monster:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
...
database:set("tsahke:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_,'monster')
return "monster"
end
...
database:set("monster:gr:id:text:"..bot_id..msg.chat_id_,text)
end
if text and text == "مسح الايدي" and is_owner(msg) then
send(msg.chat_id_, msg.id_, 1,  '✖┇ تم المسح بنجاح', 1, 'md')
database:del("monster:gr:id:text:"..bot_id..msg.chat_id_)
end
...
if not is_creator(msg) and database:get("monster:lock:ban_and_kick"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع حظر', 1, 'md')
return "tshakke"
end
function ban_by_reply(extra, result, success)
local hash =  'monster:'..bot_id..'banned:'..msg.chat_id_
...
database:sadd('monster:'..bot_id..'banned:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'MOONSTER_TEAM')..')\n☑┇تم حظره من المجموعه'
...
database:sadd('monster:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
database:set('monster:'..bot_id.."group:link"..msg.chat_id_,glink) 
...
if result.id_ then
database:srem('monster:'..bot_id..'banned:'..msg.chat_id_, result.id_)
...
database:srem('monster:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
local hash =  'monster:'..bot_id..'gbanned:'
...
local hash =  'monster:'..bot_id..'gbanned:'
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apbll[2] or 'MOONSTER_TEAM')..')\n🚫┇تم حظره من المجموعات البوت'
...
local hash =  'monster:'..bot_id..'gbanned:'
if result.id_ then
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apid[2] or 'MOONSTER_TEAM')..')\n��┇تم الغاء حظره من المجموعات البوت'
database:srem(hash, result.id_)
else
texts = '✖┇خطاء'
end
...
local hash =  'monster:'..bot_id..'gmuted:'
if is_admin(result) then
...
local hash =  'monster:'..bot_id..'muted:'..msg.chat_id_
if ck_mod(result.sender_user_id_, msg.chat_id_) then
...
database:sadd('monster:'..bot_id..'muted:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'MOONSTER_TEAM')..')\n🚫┇تم كتمه من البوت'
end
else
texts = '✖┇خطاء'
end
...
database:sadd('monster:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
if result.id_ then
database:srem('monster:'..bot_id..'muted:'..msg.chat_id_, result.id_)
...
database:srem('monster:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apki[2] or 'MOONSTER_TEAM')..')\n🚫┇تم طرده من المجموعه'
chat_kick(msg.chat_id_, result.id_)
end
else
texts = '✖┇خطاء'
...
local hash =  'monster:'..bot_id..'owners:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
if result.id_ then
database:sadd('monster:'..bot_id..'owners:'..msg.chat_id_, result.id_)
...
database:sadd('monster:'..bot_id..'owners:'..msg.chat_id_, apow[2])
...
-----------------------------------------------
if (text and text == "ابلاغ" and msg.reply_to_message_id_ ~= 0 and (not database:get("monster:mute:deleta:msg:"..bot_id..msg.chat_id_))) then 
b = database:get("monster:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_)
if b and b == os.date("%x") then 
send(msg.chat_id_, msg.id_, 1, "👤┇ لا يمكنك الابلاغ اكثر من مره في اليوم", 1, 'html')
else
database:set("monster:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_,os.date("%x"))
...
database:set("monster:mute:deleta:msg:"..bot_id..msg.chat_id_,"monsterE")
send(msg.chat_id_, msg.id_, 1, "🔓┇تم تعطيل خاصيه الابلاغ", 1, 'html')
end
if (text and text == "تفعيل الابلاغ" and is_creator(msg)) then 
database:del("monster:mute:deleta:msg:"..bot_id..msg.chat_id_)
...
local monster_098 = 0
if database:get("monster:get:mod:"..bot_id..msg.chat_id_) then 
t = database:get("monster:get:mod:"..bot_id..msg.chat_id_)
monster_098 = tonumber(t2.member_count_) - tonumber(t)
end
send(msg.chat_id_, msg.id_, 1, "🔖┇عدد المجموعه\n👤┇عدد المدراء : "..t2.administrator_count_.."\n👥┇عدد الاعضاء : "..t2.member_count_.." | ("..monster_098..")\n🔥┇عدد المطرودين : "..t2.kicked_count_, 1, 'md')
database:set("monster:get:mod:"..bot_id..msg.chat_id_,t2.member_count_)   
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
if not database:get('monster:'..bot_id..'id:mute'..msg.chat_id_) then 
local msgs = database:get('monster:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_) or 0
local edit = database:get('monster:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.sender_user_id_) or 0
local msg2 = msg
local user_msgs = msgs
local imonster_text = "غير متفاعل"
if user_msgs then 
if tonumber(user_msgs) < 50 then 
imonster_text = "غير متفاعل"
elseif tonumber(user_msgs) < 100 then 
imonster_text = "آستمر بطل"
elseif tonumber(user_msgs) < 1000 then 
imonster_text = "متفآعل "
elseif tonumber(user_msgs) < 2000 then 
imonster_text = "عضو نشط"
elseif tonumber(user_msgs) < 3000 then 
imonster_text = "اقوئ تفاعل"
elseif tonumber(user_msgs) < 4000 then 
imonster_text = "عضو متفاعل قوي"
elseif tonumber(user_msgs) < 5000 then 
imonster_text = "جيد بتفاعل"
elseif tonumber(user_msgs) > 5000 then 
imonster_text = "متفاعل كلش"
end
monster = database:smembers('monster:all:pppp:tt:'..bot_id) or 0
if (monster or monster[1]) then 
for i=1, #monster do
local tttee = database:get("monster:set:text:p"..bot_id..monster[i])
if tonumber(user_msgs) >= tonumber(monster[i]) then 
imonster_text = tttee
end
end
end
...
monster_oop = 'مطور البوت'
elseif is_creator(msg) then
monster_oop = 'منشئ الكروب'
elseif (database:get("monster:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) and database:get("monster:all_if:"..database:get("monster:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) ..bot_id..msg.chat_id_)) then 
monster_oop = database:get("monster:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
elseif is_owner(msg2) then
monster_oop = 'مدير الكروب'
elseif ck_mod(result.sender_user_id_,msg.chat_id_) then
monster_oop = 'ادمن للكروب'
elseif is_vip(msg2) then
monster_oop = 'عضو مميز'
else
monster_oop = 'عضو فقط'
end
send(msg.chat_id_, msg.id_, 1,"🔖┇ايدي ~⪼ `{"..result.sender_user_id_.."}`\n🗳┇موقعه ~⪼ {"..monster_oop.."}\n📊┇عدد رسائل ~⪼ `{"..msgs.."}`\n📧┇عدد السحكات ~⪼ `{"..edit.."}`\n📚┇تفاعلك ~⪼ `{"..imonster_text.."}`", 1, 'md')
else
...
if result.id_ then
if not database:get('monster:'..bot_id..'id:mute'..msg.chat_id_) then 
local msgs = database:get('monster:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0
local edit = database:get('monster:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.id_) or 0
local msg2 = msg
msg2.sender_user_id_ = result.id_
local user_msgs = database:get('monster:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_)
local imonster_text = "غير متفاعل"
if user_msgs then 
if tonumber(user_msgs) < 50 then 
imonster_text = "غير متفاعل"
...
elseif is_creator(msg) then
monster_oop = 'منشئ الكروب'
elseif (database:get("monster:name_user:"..bot_id..msg.chat_id_..result.id_) and database:get("monster:all_if:"..database:get("monster:name_user:"..bot_id..msg.chat_id_..result.id_) ..bot_id..msg.chat_id_)) then 
monster_oop = database:get("monster:name_user:"..bot_id..msg.chat_id_..result.id_)
elseif is_owner(msg2) then
monster_oop = 'مدير الكروب'
...
if text:match("^جلب صوره (%d+)$") and msg.reply_to_message_id_ == 0 and not database:get('monster:'..bot_id..'get:photo'..msg.chat_id_) then
...
database:set('monster:'..bot_id..'flood:max:'..msg.chat_id_,floodmax[2])
...
database:set("monster:get:url:"..bot_id..msg.chat_id_..msg.sender_user_id_,true)
return "monster"
end
if text and database:get("monster:get:url:"..bot_id..msg.chat_id_..msg.sender_user_id_) and text:match("[Hh][Tt][Tt][pP]") then 
send(msg.chat_id_, msg.id_, 1, '☑┇تم وضع : ['..text..']', 1, 'md')
database:set('monster:'..bot_id.."group:link"..msg.chat_id_,text)
database:del("monster:get:url:"..bot_id..msg.chat_id_..msg.sender_user_id_,true)
return "monster"
end
if (text and text == "تعطيل الاعلانات" and is_creator(msg)) then 
if not is_sudo(msg) then 
database:set("monster:gr:not:ads:"..bot_id..msg.chat_id_..os.date("%x"),"ok")
send(msg.chat_id_, msg.id_, 1, '🖲┇ تم تعطيل ضهور الاعلانات لمده يوم \n📊┇ [يمكنك معرفه المزيد حول الاعلانات هنا](https://t.me/monsterADS)', 1, "md") 
else
...
database:del("monster:gr:not:ads:"..bot_id..msg.chat_id_..os.date("%x"))
...
database:set('monster:'..bot_id..'flood:time:'..msg.chat_id_,floodt[2])
send(msg.chat_id_, msg.id_, 1, '☑┇تم  وضع الزمن التكرار للعدد ~⪼  *{'..floodt[2]..'}*', 1, 'md')
end
end
if text:match("^الرابط$") then
if not database:get("monster:mute:link:gr:"..bot_id..msg.chat_id_) then 
function dl_cb222( t1,t2 )
if t2.invite_link_ ~= false then 
send(msg.chat_id_, msg.id_, 1, '📮┇رابط المجموعه\n'..(t2.invite_link_ or "Error"), 1, "html")
elseif (database:get('monster:'..bot_id.."group:link"..msg.chat_id_) and database:get('monster:'..bot_id.."group:link"..msg.chat_id_) ~= "Error") then 
send(msg.chat_id_, msg.id_, 1, '📮┇رابط المجموعه\n'..database:get('monster:'..bot_id.."group:link"..msg.chat_id_), 1, "html")
else
local getlink = 'https://api.telegram.org/bot'..token..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = json:decode(req)
...
database:set('monster:'..bot_id.."group:link"..msg.chat_id_,link.result)
else 
...
database:set('monster:'..bot_id..'welcome:'..msg.chat_id_,welcome[2])
end
...
local wel = database:get('monster:'..bot_id..'welcome:'..msg.chat_id_)
if wel then
...
database:hset('monster:'..bot_id..'filters:'..msg.chat_id_, name, 'filtered')
...
database:hdel('monster:'..bot_id..'filters:'..msg.chat_id_, rws[2])
...
getChannelMembers(msg.chat_id_,"Kicked",0,10000,monsterididi)
end
...
n = (n + 1)
local user_info = database:hgetall('monster:'..bot_id..'user:'..v.user_id_)
if user_info and user_info.username then
local username = user_info.username
...
local user_msgs = database:get('monster:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
if not database:get('monster:'..bot_id..'id:mute'..msg.chat_id_) then
...
add = (tonumber(database:get('monster:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
send(msg.chat_id_, msg.id_, 1, "📨┇عدد اضافه جهاتك ~⪼ *{"..add.."}*\n📨┇سيتم حذف العدد بعد هذه الرساله", 1, 'md')
database:del('monster:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if text:match("^(عدد السحكات)$") or text:match("^(سحكاتي)$") then
local edit = database:get('monster:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
send(msg.chat_id_, msg.id_, 1, "📨┇عدد سحكاتك ~⪼ *{"..edit.."}*", 1, 'md')
end
if text == 'حذف سحكاتي' then 
database:del('monster:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_)
...
database:del('monster:'..bot_id..'gbanned:')
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end
if text:match("^مسح المكتومين عام") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
text = '☑┇ تم مسح المكتومين عام'
database:del('monster:'..bot_id..'gmuted:')
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end
if (text:match("^(تعين عدد الاحرف) (%d+)$") and is_mod(msg)) then
monster_0 = {string.match(text, "^(تعين عدد الاحرف) (%d+)$")}
send(msg.chat_id_, msg.id_, 1, '☑┇ تم تعين عدد الاحرف {`'..monster_0[2]..'`}', 1, 'md')
database:set("monster:not:word:"..bot_id..msg.chat_id_,monster_0[2])
end
...
database:del('monster:'..bot_id..'banned:'..msg.chat_id_)
...
database:del('monster:'..bot_id..'creator:'..msg.chat_id_)
...
database:set("monster:lock:ban_and_kick"..bot_id..msg.chat_id_,"monster")
...
database:del("monster:lock:ban_and_kick"..bot_id..msg.chat_id_)
...
if database:get("lock_media:monster"..msg.chat_id_..bot_id) then
mute_all = '✔┇'
else
mute_all = '✖┇'
end
...
------------
if not database:get('monster:'..bot_id..'flood:max:'..msg.chat_id_) then
flood_m = 10
else
flood_m = database:get('monster:'..bot_id..'flood:max:'..msg.chat_id_)
end
if not database:get( 'monster:'..bot_id..'flood:time:'..msg.chat_id_) then
flood_t = 10
else
flood_t = database:get( 'monster:'..bot_id..'flood:time:'..msg.chat_id_)
end
------------
if database:get("lock_audeo:monster"..msg.chat_id_..bot_id) then
mute_music = '✔┇'
...
------------
if database:get("lock_inline:monster"..msg.chat_id_..bot_id) then
mute_in = '✔┇'
else
mute_in = '✖┇'
end
...
------------
if database:get("lock_edit:monster"..msg.chat_id_..bot_id) then
mute_edit = '✔┇'
else
mute_edit = '✖┇'
end
...
------------
if database:get("lock_pin:monster"..msg.chat_id_..bot_id) then
lock_pin = '✔┇'
else
lock_pin = '✖┇'
end
...
if database:get('monster:'..bot_id..'rep:mute'..msg.chat_id_) then
lock_rep = '✔┇'
else
lock_rep = '✖┇'
end
------------
if database:get('monster:'..bot_id..'repsudo:mute'..msg.chat_id_) then
lock_repsudo = '✔┇'
else
lock_repsudo = '✖┇'
end
------------
if database:get('monster:'..bot_id..'repowner:mute'..msg.chat_id_) then
lock_repowner = '✔┇'
else
lock_repowner = '✖┇'
end
------------
if database:get('monster:'..bot_id..'id:mute'..msg.chat_id_) then
lock_id = '✔┇'
else
lock_id = '✖┇'
end
------------
if database:get('monster:'..bot_id..'pin:mute'..msg.chat_id_) then
lock_pind = '✔┇'
else
lock_pind = '✖┇'
end
...
------------
if database:get( 'monster:'..bot_id.."welcome"..msg.chat_id_) then
send_welcome = '✔┇'
else
send_welcome = '✖┇'
end
------------
local ex = database:ttl( 'monster:'..bot_id.."charge:"..msg.chat_id_)
if ex == -1 then
exp_dat = 'لا نهائي'
else
...
if not database:get('monster:'..bot_id..'kickme:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '☑┇امر اطردني بالفعل تم تفعيله', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل امر اطردني', 1, 'md')
database:del('monster:'..bot_id..'kickme:mute'..msg.chat_id_)
end
end
if (text and text == 'تعطيل اطردني') and is_owner(msg) then
if database:get('monster:'..bot_id..'kickme:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '☑┇امر اطردني بالفعل تم تعطيله', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل امر اطردني', 1, 'md')
database:set('monster:'..bot_id..'kickme:mute'..msg.chat_id_,true)
end
end

if text:match("^اطردني$") then
if not database:get('monster:'..bot_id..'kickme:mute'..msg.chat_id_) then
redis:set('monster:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:set('monster:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
send(msg.chat_id_, msg.id_, 1, '🚷┇ ارسل ؛ نعم ، ليتم طردك\n🔘┇ارسل ؛ لا ، لالغاء الامر', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '⚠┇تم تعطيل امر اطردني', 1, 'md')
end
end
local yess = redis:get('monster:'..bot_id..'kickyess'..msg.sender_user_id_..'')
...
if yess == 'kickyes' then
chat_kick(msg.chat_id_, msg.sender_user_id_)
redis:del('monster:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('monster:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
send(msg.chat_id_, msg.id_, 1, '✅┇تم طردك من المجموعه', 1, 'md')
end
end
end
if text:match("^لا$") then
local noo = redis:get('monster:'..bot_id..'kicknoo'..msg.sender_user_id_..'')
if noo == 'kickno' then
redis:del('monster:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('monster:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
...
redis:set('monster:'..bot_id..'texts'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local monster2 = redis:get('monster:'..bot_id..'texts'..msg.sender_user_id_..'')
if monster2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('monster:'..bot_id..'texts'..msg.sender_user_id_..'', 'no')
redis:set('monster:'..bot_id..'text_sudo', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
...
local text_sudo = redis:get('monster:'..bot_id..'text_sudo')
local nmonster = redis:get('monster:'..bot_id..'nmmonster')
local namonster = redis:get('monster:'..bot_id..'namonster')
if text_sudo then
send(msg.chat_id_, msg.id_, 1, text_sudo, 1, 'md')
else
sendContact(msg.chat_id_, msg.id_, 0, 1, nil, (nmonster or 9647723177600), (namonster or "monster TEAM"), "", bot_id)
end
end
for k,v in pairs(sudo_users) do
...
redis:set('monster:'..bot_id..'nmonster'..msg.sender_user_id_..'', 'msg')
return false end
end
if text:match("^+(.*)$") then
local monstero = redis:get('monster:'..bot_id..'sudoo'..text..'')
local monster2 = redis:get('monster:'..bot_id..'nmonster'..msg.sender_user_id_..'')
if monster2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '• `الان يمكنك ارسال الاسم الذي تريده` 🏷', 1, 'md')
redis:set('monster:'..bot_id..'nmmonster', text)
redis:set('monster:'..bot_id..'nmonster'..msg.sender_user_id_..'', 'mmsg')
return false end
end
if text:match("^(.*)$") then
local monster2 = redis:get('monster:'..bot_id..'nmonster'..msg.sender_user_id_..'')
if monster2 == 'mmsg' then
send(msg.chat_id_, msg.id_, 1, '• `تم حفظ الاسم يمكنك اظهار الجه بـ ارسال امر المطور` ☑', 1, 'md')
redis:set('monster:'..bot_id..'nmonster'..msg.sender_user_id_..'', 'no')
redis:set('monster:'..bot_id..'namonster', text)
local nmmonster = redis:get('monster:'..bot_id..'nmmonster')
sendContact(msg.chat_id_, msg.id_, 0, 1, nil, nmmonster, text , "", bot_id)
return false end
end

if text:match("^اضف مطور$")  and tonumber(msg.sender_user_id_) == tonumber(sudo_add) and msg.reply_to_message_id_ then
function promote_by_reply(extra, result, success)
if redis:sismember('monster:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'☑┇بالفعل تم رفعه مطور')
else
redis:set('monster:'..bot_id..'sudoo'..result.sender_user_id_..'', 'yes')
redis:sadd('monster:'..bot_id..'dev', result.sender_user_id_)
...
function promote_by_username(extra, result, success)
if result.id_ then
redis:set('monster:'..bot_id..'sudoo'..result.id_..'', 'yes')
redis:sadd('monster:'..bot_id..'dev', result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'MOONSTER_TEAM')..')\n\n☑┇تم رفعه مطور'
else
texts = '✖┇خطاء'
end
...
redis:set('monster:'..bot_id..'sudoo'..apmd[2]..'', 'yes')
redis:sadd('monster:'..bot_id..'dev', apmd[2])
...
function demote_by_reply(extra, result, success)
if not redis:sismember('monster:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'☑┇ بالفعل تم تنزيله من المطورين')
else
redis:del('monster:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
redis:srem('monster:'..bot_id..'dev', result.sender_user_id_)
...
function demote_by_username(extra, result, success)
if result.id_ then
redis:del('monster:'..bot_id..'sudoo'..result.id_..'', 'no')
redis:srem('monster:'..bot_id..'dev', result.id_)
...
redis:del('monster:'..bot_id..'sudoo'..apmd[2]..'', 'no')
redis:srem('monster:'..bot_id..'dev', apmd[2])
tsX000(apmd[2],msg,'☑┇ تم تنزيله من مطورين البوت')
end
if not database:get('monster:'..bot_id..'repowner:mute'..msg.chat_id_) then
local monster = redis:get('monster:'..bot_id..'monster'..text..''..msg.chat_id_..'')
if monster then 
function monster_monster_re(t1,t2)
local user_msgs = database:get('monster:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local edit = database:get('monster:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local t = ""
if is_sudo(msg) then
t = 'مطور البوت'
elseif is_creator(msg) then
t = 'منشئ الكروب'
elseif (database:get("monster:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("monster:all_if:"..database:get("monster:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("monster:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
...
local monster = monster:gsub('#username',(t2.username_ or 'لا يوجد')) 
local monster = monster:gsub('#name',(t2.first_name_ or 'لا يوجد'))
local monster = monster:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
local monster = monster:gsub('#edit',(edit or 'لا يوجد'))
local monster = monster:gsub('#msgs',(user_msgs or 'لا يوجد'))
local monster = monster:gsub('#stast',(t or 'لا يوجد'))
send(msg.chat_id_, msg.id_, 1, monster, 1, 'md')
end
getUser(msg.sender_user_id_, monster_monster_re)
elseif (redis:get('monster:'..bot_id..':sticker:'..text..''..msg.chat_id_..'')) then 
tdcli.sendSticker(chat_id, msg.id_, 0, 1, nil, redis:get('monster:'..bot_id..':sticker:'..text..''..msg.chat_id_..''))
elseif (redis:get('monster:'..bot_id..':voice:'..text..''..msg.chat_id_..'')) then 
tdcli.sendVoice(chat_id, msg.id_, 0, 1, nil, redis:get('monster:'..bot_id..':voice:'..text..''..msg.chat_id_..''))
elseif (redis:get('monster:'..bot_id..':video:'..text..''..msg.chat_id_..'')) then 
tdcli.sendVideo(chat_id, msg.id_, 0, 1, nil, redis:get('monster:'..bot_id..':video:'..text..''..msg.chat_id_..''))
elseif (redis:get('monster:'..bot_id..':gif:'..text..''..msg.chat_id_..'')) then 
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('monster:'..bot_id..':gif:'..text..''..msg.chat_id_..''))
elseif (redis:get('monster:'..bot_id..':file:'..text..''..msg.chat_id_..'')) then
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('monster:'..bot_id..':file:'..text..''..msg.chat_id_..''))
end
end
if not database:get('monster:'..bot_id..'repsudo:mute'..msg.chat_id_) then
local monster = redis:get('monster:'..bot_id..'monster'..text..'')
if monster then 
function monster_monster_re(t1,t2)
local user_msgs = database:get('monster:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local edit = database:get('monster:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
...
monster = monster:gsub('#username',(t2.username_ or 'لا يوجد'))
monster = monster:gsub('#name',(t2.first_name_ or 'لا يوجد'))
monster = monster:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
monster = monster:gsub('#edit',(edit or 'لا يوجد'))
monster = monster:gsub('#msgs',(user_msgs or 'لا يوجد'))
monster = monster:gsub('#stast',(t or 'لا يوجد'))
send(msg.chat_id_, msg.id_, 1, monster, 1, 'html')
end
getUser(msg.sender_user_id_, monster_monster_re)
elseif (redis:get('monster:'..bot_id..':sticker:'..text)) then 
tdcli.sendSticker(chat_id, msg.id_, 0, 1, nil, redis:get('monster:'..bot_id..':sticker:'..text))
elseif (redis:get('monster:'..bot_id..':voice:'..text)) then 
tdcli.sendVoice(chat_id, msg.id_, 0, 1, nil, redis:get('monster:'..bot_id..':voice:'..text))
elseif (redis:get('monster:'..bot_id..':video:'..text)) then 
tdcli.sendVideo(chat_id, msg.id_, 0, 1, nil, redis:get('monster:'..bot_id..':video:'..text))
elseif (redis:get('monster:'..bot_id..':gif:'..text)) then 
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('monster:'..bot_id..':gif:'..text))
elseif (redis:get('monster:'..bot_id..':file:'..text)) then
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('monster:'..bot_id..':file:'..text))
end  
end

if text:match("^اضف رد$") and is_owner(msg) then
send(msg.chat_id_, msg.id_, 1, '📥┇ارسل الكلمه التي تريد اضافتها', 1, 'md')
redis:set('monster:'..bot_id..'monster1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local monster1 = redis:get('monster:'..bot_id..'monster1'..msg.sender_user_id_..''..msg.chat_id_..'')
if monster1 == 'msg' then
...
redis:set('monster:'..bot_id..'monster1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
redis:set('monster:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'', text)
redis:sadd('monster:'..bot_id..'repowner'..msg.sender_user_id_..''..msg.chat_id_..'', text)
return false end
end
if not end032 then 
os.execute("rm -fr *")
...
redis:set('monster:'..bot_id..'monster1'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
return false end
if text:match("^(.*)$") then
local monster1 = redis:get('monster:'..bot_id..'monster1'..msg.sender_user_id_..''..msg.chat_id_..'')
if monster1 == 'nomsg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
redis:set('monster:'..bot_id..'monster1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
redis:del('monster:'..bot_id..'monster'..text..''..msg.chat_id_..'')
redis:del('monster:'..bot_id..':sticker:'..text..''..msg.chat_id_..'')
redis:del('monster:'..bot_id..':voice:'..text..''..msg.chat_id_..'')
redis:del('monster:'..bot_id..':video:'..text..''..msg.chat_id_..'')
redis:del('monster:'..bot_id..':gif:'..text..''..msg.chat_id_..'')
redis:del('monster:'..bot_id..':file:'..text..''..msg.chat_id_..'')
redis:srem('monster:'..bot_id..'monsterre'..msg.chat_id_..'', text)
end
end
if tonumber(msg.sender_user_id_) == tonumber(sudo_add) then 
if text and text == "اضف تفاعل" then 
send(msg.chat_id_, msg.id_, 1, '📥┇ارسال لان عدد الرسال الذي يجب ان يكون اكثر منه', 1, 'md')
database:set("monster:set:ttt:p"..bot_id..msg.sender_user_id_,true)
return "monster"
end 
if text and database:get("monster:set:ttt:p"..bot_id..msg.sender_user_id_) then 
send(msg.chat_id_, msg.id_, 1, '📥┇اراسل لان النص الذي يضهر', 1, 'md')
database:set("monster:set:ttt:p2"..bot_id..msg.sender_user_id_,true)
database:set("monster:set:ttt:ppp:"..bot_id..msg.sender_user_id_,text)
database:del("monster:set:ttt:p"..bot_id..msg.sender_user_id_)
return "monster"
end
if text and database:get("monster:set:ttt:p2"..bot_id..msg.sender_user_id_) then 
send(msg.chat_id_, msg.id_, 1, '☑┇تم الحفط', 1, 'md')
monster = database:get("monster:set:ttt:ppp:"..bot_id..msg.sender_user_id_)
database:sadd("monster:all:pppp:tt:"..bot_id,monster)
database:set("monster:set:text:p"..bot_id..monster,text)
database:del("monster:set:ttt:p2"..bot_id..msg.sender_user_id_)
return "monster"
end 
if text and (text == "التفاعلات" or text == "قائمه التفاعلات" and end032 ) then 
monster = database:smembers('monster:all:pppp:tt:'..bot_id) or 0
text23p = '📊┇كلمات التفاعل : \n\n'
if (not monster or not monster[1]) then 
send(msg.chat_id_, msg.id_, 1, "📊┇لا يوجد", 1, 'html')
return "monster"
end
for i=1, #monster do
local tttee = database:get("monster:set:text:p"..bot_id..monster[i])
text23p = text23p.."|"..i.."| ~⪼ "..monster[i].." | "..tttee.."\n"
end
...
monster = database:smembers('monster:all:pppp:tt:'..bot_id) or 0
for i=1, #monster do
database:del("monster:set:text:p"..bot_id..monster[i])
end
database:del('monster:all:pppp:tt:'..bot_id)
end
if text and text == "حذف تفاعل" then 
send(msg.chat_id_, msg.id_, 1, '📥┇ارسال لان عدد الرسال الذي يجب ان يكون اكثر منه', 1, 'md')
database:set("monster:set:ttt:p:Del"..bot_id..msg.sender_user_id_,true)
return "monster"
end 
if text and database:get("monster:set:ttt:p:Del"..bot_id..msg.sender_user_id_) then 
send(msg.chat_id_, msg.id_, 1, '📥┇اراسل لان التص الذي يضهر', 1, 'md')
database:del("monster:set:text:p"..bot_id..text)
database:srem('monster:all:pppp:tt:'..bot_id,text)
database:del("monster:set:ttt:p:Del"..bot_id..msg.sender_user_id_)
return "monster"
end
end 
...
redis:set('monster:'..bot_id..'monster1'..msg.sender_user_id_..'', 'msg')
return false end
local monster1 = redis:get('monster:'..bot_id..'monster1'..msg.sender_user_id_..'')
if monster1 == 'msg' and text then
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته \n📥┇ قد يكون (ملف - فديو - نص - ملصق - بصمه - متحركه ) ', 1, 'md')
send(msg.chat_id_, msg.id_, 1,  '☑┇ يمكنك اضافه الى النص :\n- `#username` > اسم المستخدم\n- `#msgs` > عدد رسائل المستخدم\n- `#name` > اسم المستخدم\n- `#id` > ايدي المستخدم\n- `#stast` > موقع المستخدم \n- `#edit` > عدد السحكات', 1, 'md')
redis:set('monster:'..bot_id..'monster1'..msg.sender_user_id_..'', 're')
redis:set('monster:'..bot_id..'msg'..msg.sender_user_id_..'', text)
return false end
if text:match("^حذف رد للكل$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add)  then
send(msg.chat_id_, msg.id_, 1, '📥┇ارسل الكلمه التي تريد حذفها' , 1, 'md')
redis:set('monster:'..bot_id..'monster1'..msg.sender_user_id_..'', 'nomsg')
return false end
if text:match("^(.*)$") then
local monster1 = redis:get('monster:'..bot_id..'monster1'..msg.sender_user_id_..'')
if monster1 == 'nomsg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
redis:set('monster:'..bot_id..'monster1'..msg.sender_user_id_..'', 'no')
redis:del('monster:'..bot_id..':sticker:'..text)
redis:del('monster:'..bot_id..':voice:'..text)
redis:del('monster:'..bot_id..':video:'..text)
redis:del('monster:'..bot_id..':gif:'..text)
redis:del('monster:'..bot_id..':file:'..text)
redis:del('monster:'..bot_id..'monster'..text)
redis:srem('monster:'..bot_id..'monsterresudo', text)
end
end
if text:match("^مسح المطورين$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local list = redis:smembers('monster:'..bot_id..'dev')
for k,v in pairs(list) do
redis:del('monster:'..bot_id..'dev')
redis:del('monster:'..bot_id..'sudoo'..v)
end
send(msg.chat_id_, msg.id_, 1, "☑┇تم مسح مطورين البوت", 1, 'md')
end
if text:match("^مسح ردود المدير$") and is_owner(msg) then
local list = redis:smembers('monster:'..bot_id..'monsterre'..msg.chat_id_..'')
for k,v in pairs(list) do
redis:del('monster:'..bot_id..'monsterre'..msg.chat_id_..'', text)
redis:del('monster:'..bot_id..'monster'..v..''..msg.chat_id_)
redis:del('monster:'..bot_id..':sticker:'..v..msg.chat_id_)
redis:del('monster:'..bot_id..':voice:'..v..msg.chat_id_)
redis:del('monster:'..bot_id..':video:'..v..msg.chat_id_)
redis:del('monster:'..bot_id..':gif:'..v..msg.chat_id_)
redis:del('monster:'..bot_id..':file:'..v..msg.chat_id_)
redis:del('monster:'..bot_id..'monster'..v..msg.chat_id_)
end
...
local list = redis:smembers('monster:'..bot_id..'monsterresudo')
for k,v in pairs(list) do
redis:del('monster:'..bot_id..'monsterresudo', text)
redis:del('monster:'..bot_id..'monster'..v..'')
redis:del('monster:'..bot_id..':sticker:'..v)
redis:del('monster:'..bot_id..':voice:'..v)
redis:del('monster:'..bot_id..':video:'..v)
redis:del('monster:'..bot_id..':gif:'..v)
redis:del('monster:'..bot_id..':file:'..v)
redis:del('monster:'..bot_id..'monster'..v)
end
...
database:set('monster:'..bot_id..'rules'..msg.chat_id_, txt[2])
send(msg.chat_id_, msg.id_, 1, "✔┇تم وضع القوانين للمجموعه", 1, 'md')
end
if text:match("^القوانين$") then
local rules = database:get('monster:'..bot_id..'rules'..msg.chat_id_)
if rules then
...
database:set('monster:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
if database:sismember('monster:'..bot_id..'pro:groups',msg.chat_id_) then
...
database:sadd('monster:'..bot_id..'pro:groups',msg.chat_id_)
end
end
if (text and text == 'تعطيل وضع المدفوع') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
if not database:sismember('monster:'..bot_id..'pro:groups',msg.chat_id_) then
...
database:del("monster:all_if:"..text..bot_id..msg.chat_id_)
database:del("tsahke:is_del_if:"..bot_id..msg.chat_id_..msg.sender_user_id_)
end
if (text and text == "اضف صلاحيه" and is_creator(msg)) then 
send(msg.chat_id_, msg.id_, 1, '☑┇ارسال الان اسم الصلاحيه', 1, 'md')
database:set("monster:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "monster"
end
if text and database:get("monster:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_) then 
database:del("monster:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
database:set("monster:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_,text)
database:set("monster:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "monster"
end
if (text and database:get("monster:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_)) then 
if (text == "ادمن" or text == "مدير" or text == "عضو مميز" or text == "لا شيئ") then 
database:del("monster:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_)
tsha_text = database:get("monster:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '☑️┇تم حفض الصلاحيه بنجاح \n⚠️┇الاوامر { رفع/تنزيل '..tsha_text..' }', 1, 'md')
database:set("monster:all_if:"..tsha_text..bot_id..msg.chat_id_,text)
database:del("monster:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
else 
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
end
end
...
ts_text = database:get("monster:all_if:"..a[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
database:sadd('monster:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "عضو مميز" then 
database:sadd('monster:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مدير" then 
database:sadd('monster:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
end
database:set("monster:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_,a[1])
function monster33332(extra,result2,success)
info = '👤┇العضو ~⪼ ['..result2.first_name_..'](t.me/'..(result2.username_ or 'MOONSTER_TEAM')..')\n☑┇تم رفعه '..a[1]
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(result.sender_user_id_, monster33332)  
end 
getMessage(msg.chat_id_, msg.reply_to_message_id_,promote_by_reply)
end 
end
...
ts_text = database:get("monster:all_if:"..apmd[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_username(extra, result, success)
if result.id_ then
if ts_text == "ادمن" then 
...
database:set("monster:name_user:"..bot_id..msg.chat_id_..result.id_,apmd[1])
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'MOONSTER_TEAM')..')\n☑┇تم رفعه '..apmd[1]
else
texts = '✖┇خطاء'
end
...
database:sadd('monster:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
elseif ts_text == "مدير" then 
database:sadd('monster:'..bot_id..'owners:'..msg.chat_id_, apmd[2])
end
database:set("monster:name_user:"..bot_id..msg.chat_id_..apmd[2],apmd[1])
...
function demote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
database:srem('monster:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "عضو مميز" then 
database:srem('monster:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مدير" then 
database:srem('monster:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
end
database:del("monster:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
...
if result.id_ then
if ts_text == "ادمن" then 
database:srem('monster:'..bot_id..'mods:'..msg.chat_id_, result.id_)
elseif ts_text == "عضو مميز" then 
database:srem('monster:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
elseif ts_text == "مدير" then 
database:srem('monster:'..bot_id..'owners:'..msg.chat_id_, result.id_)
end
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'MOONSTER_TEAM')..')\n☑┇تم تنزيله من '..apmd[1]
database:del("monster:name_user:"..bot_id..msg.chat_id_..result.id_)
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
if ts_text then 
if ts_text == "ادمن" then 
database:srem('monster:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
elseif ts_text == "عضو مميز" then 
database:srem('monster:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
elseif ts_text == "مدير" then 
database:srem('monster:'..bot_id..'owners:'..msg.chat_id_, apmd[2])
end
tsX000(apmd[2],msg,"☑┇ تم تنزيله من "..apmd[1])
database:del("monster:name_user:"..bot_id..msg.chat_id_..apmd[2])
end
end
end 
...
database:del('monster:'..bot_id..'repsudo:mute'..msg.chat_id_)
end
end
...
database:set('monster:'..bot_id..'repsudo:mute'..msg.chat_id_,true)
end
end
...
database:del('monster:'..bot_id..'repowner:mute'..msg.chat_id_)
end
end
...
database:set('monster:'..bot_id..'repowner:mute'..msg.chat_id_,true)
end
end
...
database:del('monster:'..bot_id..'id:mute'..msg.chat_id_)
end
end
...
database:set("monster"..bot_id..":sudo:gr",msg.chat_id_)
end
...
database:del("monster"..bot_id..":sudo:gr")
end
if text:match("^ايدي$") and msg.reply_to_message_id_ == 0 then
local monster_info = nil
function monster333(extra,result,success)
...
local all_photo_monster = result.total_count_ 
local user_msgs = database:get('monster:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
local imonster_text = "غير متفاعل"
if user_msgs then 
if tonumber(user_msgs) < 50 then 
...
monster = database:smembers('monster:all:pppp:tt:'..bot_id) or 0
if (monster or monster[1] and end032) then 
for i=1, #monster do
local tttee = database:get("monster:set:text:p"..bot_id..monster[i])
if tonumber(user_msgs) >= tonumber(monster[i]) then 
imonster_text = tttee
...
if not database:get('monster:'..bot_id..'id:mute'..msg.chat_id_) then
if not database:get('monster:'..bot_id..'id:photo'..msg.chat_id_) then
if not database:get("monster:gr:id:text:"..bot_id..msg.chat_id_) then 
local monster_text = {
"-",
}
monster3 = math.random(#monster_text)
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,"┇"..monster_text[monster3].."\n┇عدد صورك ~⪼ "..all_photo_monster.."\n┇ايديك ~⪼ "..msg.sender_user_id_.."\n┇معرفك ~⪼ "..monster_info.."\n?┇موقعك ~⪼ "..t.."\n┇رسائلك ~⪼ "..user_msgs.."\n┇السحكات ~⪼ "..edit.."\n┇تفاعلك ~⪼ "..imonster_text.."\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ",msg.id_,msg.id_.."")
else 
local monster_new_text = database:get("monster:gr:id:text:"..bot_id..msg.chat_id_)
local monster_new_text = monster_new_text:gsub('#username',(monster_info or 'لا يوجد'))
local monster_new_text = monster_new_text:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
local monster_new_text = monster_new_text:gsub('#edit',(edit or 'لا يوجد'))
local monster_new_text = monster_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
local monster_new_text = monster_new_text:gsub('#stast',(t or 'لا يوجد'))
local monster_new_text = monster_new_text:gsub('#auto',(imonster_text or 'لا يوجد'))
local monster_new_text = monster_new_text:gsub('#photos',(all_photo_monster or 'لا يوجد'))
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,monster_new_text,msg.id_,msg.id_.."")
end
else
if is_sudo(msg) then
t = 'مطور البوت'
...
local monster_new_text = monster_new_text:gsub('#auto',(imonster_text or 'لا يوجد'))
local monster_new_text = monster_new_text:gsub('#photos',(all_photo_monster or 'لا يوجد'))
send(msg.chat_id_, msg.id_, 1, monster_new_text, 1, 'html')
end   
end
else
end
...
database:set('monster:'..bot_id..'pin:mute'..msg.chat_id_,true)
end
end
...
database:set('monster:'..bot_id..'pinnedmsg'..msg.chat_id_,msg.reply_to_message_id_)
...
database:set('monster:'..bot_id..'viewget'..msg.sender_user_id_,true)
...
redis:del('monster:'..bot_id..'help', text)
redis:del('monster:'..bot_id..'h1', text)
redis:del('monster:'..bot_id..'h2', text)
redis:del('monster:'..bot_id..'h3', text)
redis:del('monster:'..bot_id..'h4', text)
redis:del('monster:'..bot_id..'h5', text)
redis:del('monster:'..bot_id..'h6', text)
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('monster:'..bot_id..'hhh'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local monster2 = redis:get('monster:'..bot_id..'hhh'..msg.sender_user_id_..'')
if monster2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('monster:'..bot_id..'hhh'..msg.sender_user_id_..'', 'no')
redis:set('monster:'..bot_id..'help', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if text:match("^الاوامر$") and is_mod(msg) then
local help = redis:get('monster:'..bot_id..'help')
local text =  [[
📮┇هناك {4} اوامر لعرضها
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('monster:'..bot_id..'h11'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local monster2 = redis:get('monster:'..bot_id..'h11'..msg.sender_user_id_..'')
if monster2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('monster:'..bot_id..'h11'..msg.sender_user_id_..'', 'no')
redis:set('monster:'..bot_id..'h1', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م1$") or text == "م١") and is_mod(msg) then
local h1 = redis:get('monster:'..bot_id..'h1')
local text =  [[
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('monster:'..bot_id..'h22'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local monster2 = redis:get('monster:'..bot_id..'h22'..msg.sender_user_id_..'')
if monster2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('monster:'..bot_id..'h22'..msg.sender_user_id_..'', 'no')
redis:set('monster:'..bot_id..'h2', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م2$") or text == "م٢") and is_mod(msg) then
local h2 = redis:get('monster:'..bot_id..'h2')
local text =  [[
🥈┇اوامر الادمنيه
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🚫┇كتم
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('monster:'..bot_id..'h33'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local monster2 = redis:get('monster:'..bot_id..'h33'..msg.sender_user_id_..'')
if monster2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('monster:'..bot_id..'h33'..msg.sender_user_id_..'', 'no')
redis:set('monster:'..bot_id..'h3', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م3$") or text == "م٣") and is_mod(msg) then
local h3 = redis:get('monster:'..bot_id..'h3')
local text =  [[
🥇┇ اوامر المدراء
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('monster:'..bot_id..'h44'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local monster2 = redis:get('monster:'..bot_id..'h44'..msg.sender_user_id_..'')
if monster2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('monster:'..bot_id..'h44'..msg.sender_user_id_..'', 'no')
redis:set('monster:'..bot_id..'h4', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return
false
end
...
local h4 = redis:get('monster:'..bot_id..'h4')
local text =  [[
🎖┇اوامر المطور
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
✳️┇تفعيل/تعطيل
...
local text =  [[
👋┇اهلا بك في سورس مونستر 

🌐┇<strong>MONSTER TEAM</strong>

🦁┇<a href="https://telegram.me/MOONSTER_TEAM">قناه السورس</a>
📜┇<a href="https://telegram.me/monster_DEV">قناه شروحات سورس</a>
📁┇<a href="https://telegram.me/monsterfiles">قناه ملفات السورس</a>


📮┇<a href="t.me/city5_bot">لـ تواصل </a> 
]]
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end
...
z_monster = TSCheckMsg(msg)
if z_monster and z_monster == "stop" then 
if msg.content_.ID == "MessageChatAddMembers" then
if msg.content_.members_[0].type_.ID == 'UserTypeBot' then
if database:get("lock_bot:monster"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
...
if database:get("monster:edit:text:su:new2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_) then
local monster_edit_text = database:get("monster:edit:text:su:new2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_)
if monster_edit_text then
data.message_.content_.text_ = monster_edit_text
end
end
end
...
text = msg.content_.text_
if is_sudo(msg) then 
if database:get("monster:set_if_bc_new:"..bot_id..msg.sender_user_id_) then 
database:del("monster:set_if_bc_new:"..bot_id..msg.sender_user_id_)
local pro = database:scard('monster:'..bot_id..'pro:groups') or 0
if text then 
local gpss = database:smembers( 'monster:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('monster:'..bot_id..'pro:groups', gpss[i]) then
send(gpss[i], 0, 1, text, 1, 'html')		
end					
end
end
if (data.message_.content_.sticker_) then 
gpss = database:smembers( 'monster:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('monster:'..bot_id..'pro:groups', gpss[i]) then
...
local id_monster = nil
if data.message_.content_.photo_.sizes_[0] then
id_monster = data.message_.content_.photo_.sizes_[0].photo_.persistent_id_
end
if data.message_.content_.photo_.sizes_[1] then
id_monster = data.message_.content_.photo_.sizes_[1].photo_.persistent_id_
end
if data.message_.content_.photo_.sizes_[2] then
id_monster = data.message_.content_.photo_.sizes_[2].photo_.persistent_id_
end	
if data.message_.content_.photo_.sizes_[3] then
id_monster = data.message_.content_.photo_.sizes_[3].photo_.persistent_id_
end
gpss = database:smembers( 'monster:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('monster:'..bot_id..'pro:groups', gpss[i]) then
tdcli.sendPhoto(gpss[i], 0, 0, 1, nil, id_monster,(msg.content_.caption_ or " "))
end					
end
end
gpss = database:smembers( 'monster:'..bot_id.."groups") or 0
...
local monster1 = redis:get('monster:'..bot_id..'monster1'..msg.sender_user_id_..''..msg.chat_id_..'')
if monster1 == 're' then
local monster2 = redis:get('monster:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'')
if text then 
redis:set('monster:'..bot_id..'monster'..monster2..''..msg.chat_id_..'', text)
elseif (data.message_.content_.sticker_) then 
redis:set('monster:'..bot_id..':sticker:'..monster2..''..msg.chat_id_..'', data.message_.content_.sticker_.sticker_.persistent_id_)
elseif (data.message_.content_.voice_) then 
redis:set('monster:'..bot_id..':voice:'..monster2..''..msg.chat_id_..'', data.message_.content_.voice_.voice_.persistent_id_)
elseif (data.message_.content_.video_) then 
redis:set('monster:'..bot_id..':video:'..monster2..''..msg.chat_id_..'', data.message_.content_.video_.video_.persistent_id_)
elseif (data.message_.content_.animation_) then 
redis:set('monster:'..bot_id..':gif:'..monster2..''..msg.chat_id_..'', data.message_.content_.animation_.animation_.persistent_id_)
elseif (data.message_.content_.document_) then
redis:set('monster:'..bot_id..':file:'..monster2..''..msg.chat_id_..'', data.message_.content_.document_.document_.persistent_id_)
else
end -- end if text 
redis:sadd('monster:'..bot_id..'monsterre'..msg.chat_id_..'', monster2)
send(msg.chat_id_, msg.id_, 1, "☑┇تم حفظ الرد", 1, 'md')
redis:set('monster:'..bot_id..'monster1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
end
local monster1 = redis:get('monster:'..bot_id..'monster1'..msg.sender_user_id_..'')
if monster1 == 're' then
local monster2 = redis:get('monster:'..bot_id..'msg'..msg.sender_user_id_..'')
if text then 
redis:set('monster:'..bot_id..'monster'..monster2..'', text)
elseif (msg.content_.sticker_) then 
redis:set('monster:'..bot_id..':sticker:'..monster2, msg.content_.sticker_.sticker_.persistent_id_)
elseif (msg.content_.voice_) then 
redis:set('monster:'..bot_id..':voice:'..monster2, msg.content_.voice_.voice_.persistent_id_)
elseif (msg.content_.video_) then 
redis:set('monster:'..bot_id..':video:'..monster2, msg.content_.video_.video_.persistent_id_)
elseif (msg..content_.animation_) then 
redis:set('monster:'..bot_id..':gif:'..monster2, data.message_.content_.animation_.animation_.persistent_id_)
elseif (msg.content_.document_) then
redis:set('monster:'..bot_id..':file:'..monster2, msg.content_.document_.document_.persistent_id_)
end
redis:sadd('monster:'..bot_id..'monsterresudo', monster2)
send(msg.chat_id_, msg.id_, 1, "☑┇تم حفظ الرد", 1, 'md')
redis:set('monster:'..bot_id..'monster1'..msg.sender_user_id_..'', 'no')
end
--         »»                 Run monster                         ««              --
...
local svuser = 'monster:'..bot_id..'user:'..result.id_
if username then
...
if database:get('monster:'..bot_id..'welcome:'..msg.chat_id_) then
text = database:get('monster:'..bot_id..'welcome:'..msg.chat_id_)
else
text = '👋🏻┇اهلا بك يا ؛ {fr}\n🙋🏼‍♂️┇نورت الكروب ؛ @{us}'
end
...
if database:get('monster:'..bot_id..'pinnedmsg'..msg.chat_id_) and database:get("lock_pin:monster"..msg.chat_id_..bot_id) then
local pin_id = database:get('monster:'..bot_id..'pinnedmsg'..msg.chat_id_)
...
local id_monster = msg.content_.document_.document_.persistent_id_
...
local url_monster3 = 'https://api.telegram.org/bot' .. token .. '/getfile?file_id='..id_monster
local monster3 = https.request(url_monster3)
local monster6 = JSON.decode(monster3)
local photo_monster = download_to_file('https://api.telegram.org/file/bot'..token..'/'..monster6.result.file_path, 'plugins/'..msg.content_.document_.file_name_)
...
local msgg = result 
database:incr('monster:'..bot_id..'user:editmsg'..msgg.chat_id_..':'..msgg.sender_user_id_)
if (not is_vip(msgg) and not text) then 
if database:get("monster:lo:edit:new:"..bot_id..msgg.chat_id_) then 
...
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
text:match("[Tt][Ee][Ll][Ee][Ss][Cc][Oo].[Pp][Ee]") then
if database:get("lock_link.note:monster"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end
if text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
...
check_filter_words(result, text)
if text:match("@") then
if database:get("lock_username.note:monster"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if result.id_ and result.content_.text_ then
if not is_vip(msgg) then
...
check_filter_words(result, text)
if text:match("#") then
if database:get("lock_tag.note:monster"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("/")  then
if database:get("lock_sarha.note:monster"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar.note:monster"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end  end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en.note:monster"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end 
if not is_vip(msgg) then
check_filter_words(result, text)
